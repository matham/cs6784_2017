...
class WideResNet(nn.Module):
    ...
    def __init__(self, depth, num_classes, widen_factor=1, dropRate=0.0, n_binary_class=0):
        ...
        if n_binary_class:
            self.bin_fc = nn.Linear(nChannels[3], 2)
        else:
            self.bin_fc = None
    ...
    def reset_final_layer(self):
        ...
        if self.bin_fc:
            reset_layers.append(self.bin_fc)
    ...
    def forward(self, x):
        ...
        binary_out = []
        if self.bin_fc:
            binary_out.append(F.log_softmax(self.bin_fc(out)))
